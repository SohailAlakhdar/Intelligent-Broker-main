...
import {
  ...
  SearchMainContent,
  CollapseBtn,
} from "./Styles/searchElementsStyle";
...
function Search(props) {
  ...
  const handleSlider = (event, newValue, activeThumb) => {
    if (activeThumb === 0) {
      event.target.value = [
        Math.min(newValue[0], searchData[event.target.name][1]),
        searchData[event.target.name][1],
      ];
      handleChange(event);
    } else {
      event.target.value = [
        searchData[event.target.name][0],
        Math.max(newValue[1], searchData[event.target.name][0]),
      ];
      handleChange(event);
    }
  };
      ...
      {(context) => {
        ...
        return (
          <SearchMainContainer>
            ...
            <SearchMainContent>
              <ProductMainH1>Intelligent Broker</ProductMainH1>
              {validation ? (
                <ProductMainH1>
                  ...
                  {expandSearch && (
                    <CollapseDiv>
                      ...
                      <Typography variant="h5" style={CollapseBtn} gutterBottom>
                        Price range :{searchData.price[0]} to{" "}
                        {searchData.price[1]}
                        <Slider
                          min={1}
                          step={1000}
