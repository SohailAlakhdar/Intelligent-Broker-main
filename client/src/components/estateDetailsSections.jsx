...
function EstateDetailsSections(props) {
  ...
  return (
    <EstateCardDivCard className={!props.compareMode && "expandClass"}>
      ...
      {CheckAuth() ? (
        <>
          {props.data.type.name === "Auction" && (
            <EstateAuctionSection data={props.data} />
          )}
          ...
          {props.userId === props.data.sellerId ? (
            <DetailsBtnCard>
              <Button
                color="error"
                onClick={() => handelDeleteBtn(props.data._id)}
                variant="outlined"
      ...
      <Button
        color="primary"
        onClick={() => props.handleDetailsAndCompare("compare", props.data)}
        variant="outlined"
